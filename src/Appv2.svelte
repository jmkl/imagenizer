<script>
  ...
  const texture_url = "http://localhost:8811";
  ...
  */
  let all_textures = [];
  async function fetchTextureCategory(category) {
  ...
  function initTexture() {
    ...
    fetch(`${texture_url}/textures/category/FREETEXTURE`)
      .then((response) => {
        if (response.ok) return response.json();
      })
      .then((result) => {
      texture_categories = result.map(data=>data.Filename)
  });
  }
      ...
      {#if all_contents}
        ...
        {#if visible_content}
          ...
          {:else}
            ...
            >
              {#each visible_content as content }
